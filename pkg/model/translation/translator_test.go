...
func TestSharedIngressTranslator_getServices(t *testing.T) {
	...
	tests := []struct {
		...
		fields fields
		args   args
		want   []*dolphinv1.ServiceListener
	}{
		{
			name: "default case",
			fields: fields{
...
func TestSharedIngressTranslator_getListenerProxy(t *testing.T) {
	i := &defaultTranslator{
		name:             "dolphin-ingress",
		namespace:        "kube-system",
		secretsNamespace: "dolphin-secrets",
...
func TestSharedIngressTranslator_getListener(t *testing.T) {
	...
	require.Equal(t, downStreamTLS.CommonTlsContext.TlsCertificateSdsSecretConfigs[0].GetName(), "dolphin-secrets/dummy-namespace-dummy-secret")
